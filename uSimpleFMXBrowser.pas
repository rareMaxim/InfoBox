...
type
  ...
  protected
    ...
    function PostCustomMessage(aMessage: Cardinal; wParam: Cardinal = 0;
...
procedure TSimpleFMXBrowserFrm.FMXChromium1BeforeClose(Sender: TObject;
  const browser: ICefBrowser);
begin
  FCanClose := True;
...
procedure TSimpleFMXBrowserFrm.FMXChromium1BeforeContextMenu(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
  const params: ICefContextMenuParams; const model: ICefMenuModel);
begin
...
function TSimpleFMXBrowserFrm.PostCustomMessage(aMessage, wParam: Cardinal;
...
{$IFDEF MSWINDOWS}
  ...
  Result := (TempHWND <> 0) and Winapi.Windows.PostMessage(TempHWND, aMessage,
    wParam, lParam);
{$ELSE}
  Result := False;
{$ENDIF}
...
procedure TSimpleFMXBrowserFrm.FormDestroy(Sender: TObject);
begin
  FConfig.UpdateFile;
  FConfig.Free;
end;

procedure TSimpleFMXBrowserFrm.FormCloseQuery(Sender: TObject;
...
begin
  FConfig := TMemIniFile.Create('config.ini');
  FMainUrl := FConfig.ReadString('URL', 'Main', 'https://www.cnap.mlt.gov.ua/');
...
begin
  Result.Left := 0;
  Result.Top := round(Layout1.Height);
...
procedure TSimpleFMXBrowserFrm.Button2Click(Sender: TObject);
begin
  FMXChromium1.LoadURL(FMainUrl);
end;
...
begin
  ...
  begin
    ...
    FMXChromium1.DefaultUrl := FMainUrl;
...
begin
  ...
  if not(FMXChromium1.CreateBrowser(TempHandle, TempRect)) and
    not(FMXChromium1.Initialized) then
    Timer1.Enabled := True;
end;
